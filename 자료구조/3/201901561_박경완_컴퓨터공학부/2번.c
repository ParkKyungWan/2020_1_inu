		...
		for (w = 0; w < g->n; w++) {
			if (!found[w]) {
				if (distance[u] + g->weight[u][w] < distance[w]) {
					...
					u가 start 와 w 사이의 최단거리에서 들르게 되는 (w 와 가장 가까운) 중간점이 됩니다.
					*/
					if(u) between[start][w] = u;
					else between[start][w] = -1;
					//따라서 u를 중간점으로 기록해 줍니다.

					// * between이 0일경우 (A 일경우) false 로 인식되어 무시되는 오류를 발견했습니다.
					//   일단 -1로 기록하고, 꺼내쓸 때 0으로 변환하여 사용합니다. (Line 173)
	...
	for (int i = 0; i < 9; i++) {
		...
		for (int j = 0; j < 9; j++) {
			if (i != j) {             // i 와 j 가 같은 경우 무시합니다.
				...
				*/
				while (next) {
					if (next == -1) next = 0; // -1은 사실 0이므로 변환해줍니다 (line 97)
					count[now][next]++;
					now = next;
					printf("-> %c ", alpha[now]);
					next = between[i][next];
